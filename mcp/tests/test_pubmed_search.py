...
class TestSearchPubmed:
    ...
    def test_pagination_fetches_all_pmids(self, mock_req, mock_abs):
        page1_ids = [str(i) for i in range(1, 201)]
        page2_ids = [str(i) for i in range(201, 351)]
        mock_req.side_effect = [
            self._esearch_response(page1_ids, count=350),
            self._esearch_response(page2_ids, count=350),
            self._esummary_response(
                {
                    pid: {"title": f"Paper {pid}", "pubdate": "2022", "articleids": []}
        ...
        mock_abs.return_value = {}
        result = search_pubmed("q", limit=500)
        assert result["returned"] == 350
        assert result["total_in_pubmed"] == 350

    def test_truncated_flag_when_total_exceeds_limit(self, mock_req, mock_abs):
        mock_req.side_effect = [
            self._esearch_response(["1", "2"], count=550),
            self._esummary_response(
                {
                    "1": {"title": "P1", "pubdate": "2022", "articleids": []},
                    "2": {"title": "P2", "pubdate": "2022", "articleids": []},
                }
            ),
        ]
        mock_abs.return_value = {}
        result = search_pubmed("q", limit=2)
        assert result["truncated"] is True
        assert result["total_in_pubmed"] == 550
