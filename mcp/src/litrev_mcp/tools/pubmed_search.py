...
def search_pubmed(
    ...
    *,
    ...
    search_params["retmax"] = min(limit, BATCH_SIZE)
    ...
    pmids = list(esearch.get("idlist", []))

    while len(pmids) < min(total, limit):
        search_params["retstart"] = len(pmids)
        search_params["retmax"] = min(limit - len(pmids), BATCH_SIZE)
        try:
            resp = request_with_retry(
                "GET",
                ESEARCH_URL,
            ...
            resp.raise_for_status()
            page = resp.json().get("esearchresult", {}).get("idlist", [])
            if not page:
                break
            pmids.extend(page)
        except Exception as e:
            print(
                f"  Warning: esearch pagination failed at offset {len(pmids)}: {e}",
                file=sys.stderr,
            )
            break
