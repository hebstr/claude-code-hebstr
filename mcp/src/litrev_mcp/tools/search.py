...
def _format_markdown(results: list[dict], top_n: int = 0) -> str:
    ...
    for r in results:
        ...
        if isinstance(st, list):
            st = "; ".join(st) if st else "unspecified"
...
def _check_relevance(
    ...
    max_offtopic_pct: float = 25.0,
) -> dict:
    """Check per-source relevance against required terms.

    Returns dict with per-source stats and overall pass/fail.
    An article is on-topic if ANY required term appears in title+abstract.
    """
    ...
    for idx, r in enumerate(results):
        ...
        if source not in by_source:
            by_source[source] = {"total": 0, "on_topic": 0, "off_topic_indices": []}
        by_source[source]["total"] += 1
        text = ((r.get("title") or "") + " " + (r.get("abstract") or "")).lower()
        if any(t in text for t in terms_lower):
            by_source[source]["on_topic"] += 1
        else:
            by_source[source]["off_topic_indices"].append(idx)

    blocked_sources = []
    source_stats = {}
    for source, counts in by_source.items():
        total = counts["total"]
        on_topic = counts["on_topic"]
        off_topic_pct = ((total - on_topic) / total * 100) if total > 0 else 0
        source_stats[source] = {
            "total": total,
            "on_topic": on_topic,
            "off_topic_pct": round(off_topic_pct, 1),
        }
        if off_topic_pct > max_offtopic_pct:
    ...
    for source, counts in by_source.items():
        ...
        on_topic = counts["on_topic"]
        off_topic_pct = ((total - on_topic) / total * 100) if total > 0 else 0
        source_stats_detail[source] = {
            "total": total,
            "on_topic": on_topic,
            "off_topic_pct": round(off_topic_pct, 1),
            "off_topic_indices": counts["off_topic_indices"],
        }

    return {
        "passed": len(blocked_sources) == 0,
        "blocked_sources": blocked_sources,
        "source_stats": source_stats,
        "source_stats_detail": source_stats_detail,
        "max_offtopic_pct": max_offtopic_pct,
...
def process_results(
    ...
    *,
    ...
    if required_terms:
        relevance = _check_relevance(results, required_terms, max_offtopic_pct)
        log_lines.append(
            f"Relevance check (terms={required_terms}, "
            f"max_offtopic={max_offtopic_pct}%): "
            + "; ".join(
                f"{src}: {s['on_topic']}/{s['total']} on-topic ({s['off_topic_pct']}% off-topic)"
                for src, s in relevance["source_stats"].items()
            )
        )
        if not relevance["passed"]:
            if relevance_mode == "filter":
                off_topic_indices: set[int] = set()
                for src in relevance["blocked_sources"]:
                    off_topic_indices.update(
                        relevance["source_stats_detail"][src]["off_topic_indices"]
                    )
                before_filter = len(results)
                results = [
                    r for i, r in enumerate(results) if i not in off_topic_indices
                ]
                log_lines.append(
                    f"Relevance filter: removed {before_filter - len(results)} "
                    f"off-topic articles from {', '.join(relevance['blocked_sources'])}"
                )
            else:
                return {
                    "status": "blocked",
                    "reason": "relevance_gate",
                    "blocked_sources": relevance["blocked_sources"],
                    "source_stats": relevance["source_stats"],
                    "max_offtopic_pct": max_offtopic_pct,
                    "required_terms": required_terms,
                    "log": log_lines,
                    "total_results": len(results),
    ...
    if study_types:
        ...
        results = [
            ...
            or any(
                t.lower() in types_lower
                for t in (
                    r.get("publication_type")
                    if isinstance(r.get("publication_type"), list)
                    else [r.get("publication_type") or ""]
                )
            )
        ]
