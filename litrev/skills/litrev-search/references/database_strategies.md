...
S2 search is keyword-based with **implicit OR** — multi-term queries return papers matching ANY term, not all of them. Empirical observation (example-5, melatonin/sleep/autism): **63.9% off-topic** vs 7% for PubMed on the same topic. To mitigate:
- Use **short, specific queries** (2-3 terms, most discriminant concept first)
- Rely on `process_results` relevance gate (`required_terms`) to filter results post-hoc
- Prefer S2 for **citation chaining** (forward/backward from PubMed hits) over direct keyword search
...
OpenAlex search uses **implicit OR** with no Boolean or field-scoped support, AND sorts by citation count rather than relevance. Empirical observation (example-5, melatonin/sleep/autism): **88.9% off-topic** vs 7% for PubMed on the same topic. This is the worst precision of the three free APIs. To mitigate:
- Use **very short queries** (2 terms max) with the most specific concept first
- Always apply `process_results` relevance gate (`required_terms`) — expect high rejection rates
- Prefer OpenAlex for **citation enrichment by DOI/PMID** and **citation chaining**, not direct keyword search
