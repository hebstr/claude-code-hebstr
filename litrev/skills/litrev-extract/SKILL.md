...
1. **Semantic claims**: Read the abstract and identify the main findings that go beyond raw numbers. Express each claim as a concise statement (e.g., "Smoking is associated with increased rotator cuff tear risk"). **Cross-reference with regex claims**: before writing semantic claims, check the article's `claims` array from Step 2. If a regex claim contains a statistic that matches a semantic finding (e.g., the regex extracted "OR 2.1 (95% CI 1.4-3.2)" and the semantic claim is about smoking and RC tear risk), copy the statistic into the semantic claim's `effect_size` field. Every article with regex claims should have at least one semantic claim with `effect_size` populated. Assign each claim a type from this non-exhaustive list:
...
3. **Effect direction**: For association/efficacy claims, note the direction: `positive`, `negative`, `null`, or `mixed`. **Direction calibration**: do NOT default to "positive". Read the abstract carefully. Assign `null` when the study reports no significant effect or states "evidence is limited/insufficient". Assign `mixed` when the abstract reports conflicting results across outcomes or subgroups. Assign `negative` for harmful effects or inverse associations. Only assign `positive` when the abstract explicitly reports a beneficial effect or positive association with supporting data. For systematic reviews and meta-analyses, base the direction on the pooled result, not on individual studies mentioned.

4. **Optional fields**: When available in the abstract or cross-referenced from regex claims, include `effect_size` (e.g., "OR 2.1 (95% CI 1.4-3.2)") and `population` (e.g., "adults over 50"). Omit if not reported.

5. **`provides_primary_data`**: Set to `true` for studies that report original empirical data (RCTs, cohort studies, cross-sectional surveys, case-control, case series, qualitative studies with original data collection). Set to `false` for narrative reviews, editorials, commentaries, opinion pieces, guidelines, and other non-primary sources. For systematic reviews and meta-analyses, set to `false` (they synthesize existing data, not generate new primary data). This field is required for every article.

Update each article entry in the JSON with a `semantic_claims` array and a `provides_primary_data` boolean alongside the existing `claims` array (which holds the raw quantitative extractions). Write the enriched file back to `review/extracted_claims.json`.
...
- `semantic_claims` — enriched claims with type, rank, direction, effect_size (cross-referenced from regex claims)
- `provides_primary_data` — boolean distinguishing primary empirical studies from secondary sources
- `quality` — quality assessment object (with `bias_risk`, `overall_rating`, `recommended_tool`, etc.)
- `themes` — list of theme assignments

Recompute `stats` from the actual arrays before writing: set `total_quantitative_claims` to the sum of all `claims` array lengths across articles, and `total_semantic_claims` to the sum of all `semantic_claims` array lengths. Also recount `with_claims` as the number of articles where either array is non-empty. Count `primary_data_articles` as the number of articles with `provides_primary_data: true`. Do not trust the values written by `extract_claims_regex` — they reflect pre-enrichment counts.

Validate direction distribution: count the number of claims per direction value (positive, negative, null, mixed). If more than 75% of claims have the same direction, flag this in the summary as a potential calibration issue requiring manual review.
