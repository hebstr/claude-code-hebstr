...
```
{
  "stats": { "total", "with_abstract", "with_claims", "total_quantitative_claims", "total_semantic_claims", "primary_data_articles" },
...
Key points: articles are keyed by BibTeX key (`LastName_Year`). Each article has a `claims` array (quantitative, regex-extracted) and a `semantic_claims` array (LLM-enriched). Theme assignment is per-article via the `themes` list. The `provides_primary_data` boolean distinguishes primary empirical studies from secondary sources (reviews, editorials) — when synthesizing, give more evidential weight to primary data sources and note when a claim is supported only by secondary sources.
