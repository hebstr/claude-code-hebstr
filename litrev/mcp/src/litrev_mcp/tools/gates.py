...
def _gate_3b(d: str) -> list[dict]:
    ...
    snowball_match = re.search(
        r"(?i)(?:##\s*citation\s*snowball|##\s*snowball)(.*?)(?=\n##\s|\Z)",
        text,
        re.DOTALL,
    )
    snowball_text = snowball_match.group(1) if snowball_match else text

    has_retained = bool(re.search(r"(?i)###\s*retained", snowball_text))
    has_excluded = bool(re.search(r"(?i)###\s*excluded", snowball_text))
    checks.append(
        _check(
            "snowball screening documented (Retained + Excluded sections)",
            has_retained and has_excluded,
            "retained and excluded sections found"
            if has_retained and has_excluded
            else f"missing: {'Retained' if not has_retained else ''}"
            f"{'Excluded' if not has_excluded else ''} section(s) — "
            "snowball candidates may not have been screened",
        )
    )
