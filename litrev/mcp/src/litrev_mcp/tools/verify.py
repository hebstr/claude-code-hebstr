...
_STAT_LABELS = frozenset(
    "or rr hr ci auc auroc p n sd se mean median iqr roc wmd smd".split()
)
_STOPWORDS = frozenset(
    "the a an of in for and to with was were is are by from at on this that "
    "than as be had has have not no between among after during each than "
    "significantly associated increased decreased higher lower".split()
)


def _context_words(text: str) -> set[str]:
    words = set(re.findall(r"[a-z]{3,}", text.lower()))
    return words - _STAT_LABELS - _STOPWORDS


def _label_matches(review_context: str, claim_verbatim: str) -> bool:
    if not claim_verbatim:
        return True
    review_words = _context_words(review_context)
    claim_words = _context_words(claim_verbatim)
    if not review_words or not claim_words:
        return True
    overlap = review_words & claim_words
...
def audit_claims(
    ...
    *,
        ...
        if resolved is None:
            ...
            for ac in all_claims:
                ...
                if _number_matches(claim["value"], ac_value):
                    ...
                    verbatim = ac.get("verbatim") or ac.get("claim") or ac_value
                    if _label_matches(claim["context"], verbatim):
                        entry["status"] = "VERIFIED"
                        summary["verified"] += 1
                    else:
                        entry["status"] = "LABEL_UNCERTAIN"
                        entry["detail"] = (
                            "Number matches but context words differ — "
                            "verify predictor/label correspondence"
                        )
                        summary["label_uncertain"] += 1
                    break
            if not matched:
    ...
    return {
        ...
        "label_uncertain": summary["label_uncertain"],
