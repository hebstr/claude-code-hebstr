...
def process_results(
    ...
    relevance_mode: Literal["block", "filter"] = "block",
) -> dict:
    ...
    When required_terms is provided, checks per-source relevance before
    proceeding. Behavior depends on relevance_mode:
    - "block" (default): if any source exceeds max_offtopic_pct, returns
      status="blocked" with diagnostics — no output written.
    - "filter": removes individual off-topic articles from sources that
      exceed the threshold, keeps on-topic articles from all sources.
    ...
    Args:
        ...
        required_terms: Terms that must appear in title or abstract for an article to be on-topic (e.g. ["melatonin", "autism"])
        max_offtopic_pct: Max percentage of off-topic articles per source before blocking (default: 25.0)
        relevance_mode: "block" to reject entire noisy sources, "filter" to keep only on-topic articles from them (default: "block")
    """
...
def search_pubmed(
    ...
    and abstracts. Paginates esearch when limit > 200. Uses NCBI_API_KEY
    if configured (10 req/s vs 3 req/s). Returns truncated=true when
    total_in_pubmed exceeds the requested limit.
...
def audit_claims(
    ...
    Statuses: VERIFIED (found in abstract with matching context), LABEL_UNCERTAIN
    (number found but context words differ — verify predictor/label correspondence),
    UNVERIFIED (not found — may be from full-text or hallucinated), NO_ABSTRACT,
    NO_EXTRACTION. Multi-citation claims (sentence cites >1 source) are flagged
    with multi_citation=true — attribution is ambiguous.
