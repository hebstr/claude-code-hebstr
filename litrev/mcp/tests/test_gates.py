...
class TestGate3b:
    ...
    def test_fail_without_excluded_section(self, review_dir):
        _write(
            review_dir,
            "screening_log.md",
        ...
        assert result["status"] == "FAIL"
        failed = [c for c in result["checks"] if not c["passed"]]
        assert any("excluded" in c["detail"].lower() for c in failed)

    def test_pass_with_retained_and_excluded(self, review_dir):
        _write(
            review_dir,
            "screening_log.md",
            "## Citation Snowballing\n### Retained (5)\n| Index | Title |\n### Excluded (12)\n| Title | Reason |\nStatus: COMPLETE\n",
