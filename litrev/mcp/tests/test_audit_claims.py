...
class TestExtractReviewClaims:
    ...
    def test_multi_citation_flagged(self):
        text = "Biomarkers showed promise (p=0.01) [@A_2020; @B_2021; @C_2022].\n"
        claims = _extract_review_claims(text)
        assert len(claims) == 3
        assert all(c.get("multi_citation") is True for c in claims)

    def test_single_citation_not_flagged(self):
        text = "Effect was significant (p=0.01) [@A_2020].\n"
        claims = _extract_review_claims(text)
        assert len(claims) >= 1
        assert all(c.get("multi_citation", False) is False for c in claims)
...
class TestAuditClaims:
    ...
    def test_label_mismatch_flagged(self):
        review_text = (
            "## Results\n\nAge was the strongest predictor with OR 2.94 [@Yang_2017].\n"
        )
        extraction = {
            "articles": {
                "Yang_2017": {
                    ...
                    "claims": [
                        {
                            "value": "OR 2.94 (95% CI 1.25-6.93)",
                            "verbatim": "living in institution was associated with increased risk (OR 2.94, 95% CI 1.25-6.93)",
                            "type": "statistic",
                        },
                    ],
                    "semantic_claims": [],
        ...
        tmpdir, review_path, claims_path, output_path = _setup_files(
            review_text=review_text, extraction=extraction
        )
        try:
            audit_claims(review_path, claims_path=claims_path, output_path=output_path)
            with open(output_path) as f:
                audit = json.load(f)
            matched = [c for c in audit["claims"] if "2.94" in c["value"]]
            assert len(matched) >= 1
            assert matched[0]["status"] == "LABEL_UNCERTAIN"
        finally:
            for f in [review_path, claims_path, output_path]:
                if os.path.isfile(f):
                    os.unlink(f)
    ...
    def test_label_match_stays_verified(self):
        review_text = (
            "## Results\n\n"
            "Living in an institution was associated with OR 2.94 [@Yang_2017].\n"
        )
        extraction = {
            "articles": {
                "Yang_2017": {
