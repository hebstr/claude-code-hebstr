...
When the report came from `blindspot-review` (Step 1 detected a `### Convergence Analysis` section), each finding carries a bucket tag. Apply this routing on top of the standard severity rules:
...
Call `ouroboros_measure_drift` with: `session_id` ("review-walkthrough"), `current_output` (codebase state summary after fixes), `seed_content` (see fallback rules below).
