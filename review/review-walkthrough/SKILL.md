...
Match by the finding's stated category if available, otherwise by keyword in the finding's description. When in doubt, classify as excluded — the cost of a false positive (one extra rejected pattern not memorized) is much lower than the cost of a false negative (a security category silently disabled).
