...
- `deduplicate`: `true`
- `top_n`: `20`
- `required_terms`: list of 1-3 core concepts that MUST appear in title or abstract for an article to be on-topic (e.g. `["melatonin", "autism"]`). Derive these from the PICO — pick the most distinctive terms from Population and Intervention.
- `max_offtopic_pct`: `25.0` (default). Maximum percentage of off-topic articles allowed per source before triggering the relevance gate.
- `relevance_mode`: `"block"` (default) or `"filter"`.
  - `"block"`: if any source exceeds the threshold, returns `status: "blocked"` with per-source diagnostics — no output written.
  - `"filter"`: removes individual off-topic articles from sources that exceed the threshold while keeping their on-topic articles. Use this when you want to salvage on-topic results from noisy sources rather than discarding the entire source.

**Relevance gate behavior**: when `required_terms` is provided, the tool checks each source separately. If a source is blocked (>25% off-topic) and `relevance_mode` is `"block"`, you must either:
1. Reformulate the query for that source and re-run the search
2. Remove that source's results from `combined_results.json` and document the exclusion in `search_log.md`
3. If the minimum database count (3 for systematic, 2 for rapid) is still met without the blocked source, proceed without it
4. Switch to `relevance_mode: "filter"` to keep on-topic articles from the blocked source while discarding off-topic ones

Do NOT raise `max_offtopic_pct` to bypass the gate — fix the queries or use filter mode instead.
